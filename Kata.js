...
function greet(name) {
    ...
    if (name === null || typeof(name) === "undefined") {
        ...
        if (name.length === 2) {
            ...
            const normalName = name.filter((value, index, array) => { return value !== value.toUpperCase() });
            const uppercaseName = name.filter((value, index, array) => { return value === value.toUpperCase() });
